...
class BoggleboardgeneratorValidationException implements Exception {
  final List<String> errors;

  BoggleboardgeneratorValidationException(this.errors);

  @override
  String toString() => 'BoggleboardgeneratorValidationException: ${errors.join("; ")}';
}
...
/// Boggle Board Generator API client.
///
/// For more information, visit: https://apiverve.com/marketplace/boggleboardgenerator?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [size] - Board size
class BoggleboardgeneratorClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'size': ValidationRule(type: 'integer', required: false),
  };

  BoggleboardgeneratorClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/boggleboardgenerator',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [BoggleboardgeneratorValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw BoggleboardgeneratorValidationException(errors);
    }
  }

  /// Execute a request to the Boggle Board Generator API.
  ///
  /// Parameters are validated before sending the request.
  Future<BoggleboardgeneratorResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw BoggleboardgeneratorException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return BoggleboardgeneratorResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw BoggleboardgeneratorException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw BoggleboardgeneratorException('Resource not found');
      } else {
        throw BoggleboardgeneratorException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is BoggleboardgeneratorException) rethrow;
      throw BoggleboardgeneratorException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Boggle Board Generator API client.
class BoggleboardgeneratorException implements Exception {
  final String message;

  BoggleboardgeneratorException(this.message);

  @override
  String toString() => 'BoggleboardgeneratorException: $message';
}
